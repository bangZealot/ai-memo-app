...
메모 CRUD 기능과 AI 요약 기능을 제공하는 Next.js 웹 애플리케이션. Supabase Postgres를 사용하여 데이터를 영속화하며, 카테고리 분류 및 검색 기능을 지원한다.
...
- **Persistence:** Supabase Postgres via Server Actions
...
2. Supabase service role key는 서버 전용 코드에서만 사용할 것
...
- 데이터 변경은 `src/app/actions/` 서버 액션을 통해 수행
...
- 브라우저 컴포넌트에서 Supabase service role key 사용 금지
...
- **[유틸리티 (Utils)](./src/utils/AGENTS.md)** — 순수 헬퍼 함수 작업 시
