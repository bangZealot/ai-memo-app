...
클라이언트 상태 관리를 위한 커스텀 React 훅 집합. 서버 액션 호출, 메모 CRUD 상태, 검색, 필터링, 요약 요청 로직을 캡슐화한다.

## 의존성 관계

- `@/types/memo` — Memo, MemoFormData 타입
- `@/app/actions/memos` — Supabase CRUD 서버 액션
- `@/app/actions/summarize` — Gemini 요약 서버 액션
...
| `useSummarize.ts` | 메모 요약 요청 관리 | summary, loading, error, summarize, reset |
...
- 훅 이름은 `use` 접두사 필수
- Supabase CRUD는 `@/app/actions/memos` 서버 액션을 통해서만 호출
